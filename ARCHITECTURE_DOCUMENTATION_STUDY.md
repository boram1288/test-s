...
소프트웨어를 만들 때는 기능뿐 아니라 변경 용이성, 성능, 보안, 가용성 같은 품질도 결정해야 한다. 이런 품질은 특정 함수 하나가 아니라 시스템의 큰 구조에 영향을 받는다. 예를 들어 주문 처리와 알림을 동기 호출로 묶으면 구현은 단순해지지만, 알림 장애가 주문 실패로 전파될 수 있다.

이처럼 이후 구현과 운영을 크게 제약하는 결정을 **아키텍처 결정**이라고 볼 수 있다. 결정이 여러 사람의 업무에 영향을 주므로 머릿속이나 회의 기록에만 남겨서는 안 된다.
...
문서의 목적은 그림을 많이 만드는 것이 아니다. 이해관계자가 다음과 같은 결정을 내릴 수 있게 하는 것이다.
