...
> **전체 흐름: 중요한 결정을 공유해야 한다 → 한 그림으로는 부족하다 → 이해관계자에 맞는 View를 고른다 → 구조와 동작을 명확하게 기록한다 → View를 연결한다 → 독자가 사용할 수 있는지 검증한다.**
...
> **Architectural Decision은 여러 사람이 함께 참조할 수 있도록 문서화해야 한다.**

이처럼 이후 구현과 운영을 크게 제약하는 결정을 **Architectural Decision**이라고 볼 수 있다. 이러한 결정은 개발·테스트·배포·운영 등 여러 사람의 업무에 영향을 주므로 머릿속이나 회의 기록에만 남겨서는 안 된다.
...
![질문이 다르면 필요한 View도 다르다](diagrams/08-why-multiple-views.png)

> **서로 다른 관심사를 한 그림에 모두 넣지 말고 View로 분리해야 한다.**

시스템을 하나의 거대한 그림에 모두 넣으면 서로 다른 의미의 상자와 선이 섞인다. 그림이 복잡해질 뿐 아니라, 같은 화살표가 코드 의존성인지 런타임 호출인지 알 수 없게 된다.

![아키텍처를 바라보는 세 가지 핵심 Viewtype](diagrams/01-viewtypes.png)

### View, Viewtype, Style

> **View는 실제 표현이고, Viewtype과 Style은 그 표현을 구성하는 규칙이다.**

- **View**: 특정 이해관계자의 관심사를 위해 실제 시스템 일부를 표현한 결과물
- **Viewtype**: 표현할 요소와 관계의 종류를 정한 범주
- **Style**: 한 Viewtype 안에서 사용할 요소·관계·제약을 더 구체화한 규칙

### PDF에 제시된 Viewtype별 Style

| Viewtype | Style 또는 Style 계열 | 핵심 요소와 관계 | 적용 예시 |
|---|---|---|---|
...
| 컴포넌트-커넥터(C&C) | 데이터스트림(Datastream) | 필터, 파이프, 비동기 데이터 스트림 | 주문 데이터가 검증·변환·저장 필터를 순서대로 통과한다. 대표 세부 Style은 pipe-filter다. |
| 컴포넌트-커넥터(C&C) | 호출-반환(Call-Return) | 서비스 제공자·요청자와 request-reply 관계 | Web App이 Order Service를 동기 호출한다. 대표 세부 Style은 client-server와 peer-to-peer다. |
| 컴포넌트-커넥터(C&C) | 공유 데이터(Shared-Data) | 저장소, 데이터 접근자, 읽기·쓰기 관계 | 여러 서비스가 공용 저장소를 통해 데이터를 공유한다. 대표 세부 Style은 repository와 blackboard다. |
| 컴포넌트-커넥터(C&C) | 발행-구독(Publish-Subscribe) | 발행자·구독자와 이벤트 버스 | Order Service가 `OrderPlaced`를 발행하면 여러 워커가 구독한다. 세부 Style로 implicit invocation과 event-only가 있다. |
...
## 3. 질문에 따라 세 가지 Viewtype을 선택한다

> **세 Viewtype은 서로 다른 질문에 답하므로 필요한 경우 함께 사용할 수 있다.**

세 Viewtype은 하나만 선택해야 하는 경쟁 관계가 아니다.

### 3.1 모듈 Viewtype: 코드는 어떻게 나뉘는가

> **모듈 View는 구현 단위와 정적 관계를 보여준다.**
...
### 3.2 컴포넌트-커넥터 Viewtype: 실행 중 어떻게 협력하는가

> **컴포넌트-커넥터(C&C) View는 런타임 요소와 상호작용을 보여준다.**

![온라인 서점 런타임 컴포넌트-커넥터 View](diagrams/03-runtime-view.png)
...
주문 서비스가 결제 게이트웨이를 호출하고 이벤트 브로커에 주문 이벤트를 발행하는 구조는 C&C View로 표현한다.

### 3.3 할당 Viewtype: 소프트웨어는 환경과 어떻게 대응되는가

> **할당 View는 소프트웨어 요소를 소프트웨어가 아닌 환경에 대응시킨다.**

![온라인 서점 배포 할당 View](diagrams/04-deployment-view.png)

- 배포 Style: 서비스와 서버·컨테이너의 대응
- 구현 Style: 모듈과 저장소·디렉터리의 대응
- 작업 배정 Style: 모듈과 개발팀의 대응
- 답하는 질문: 어디에서 실행되는가? 코드는 어디에 있는가? 누가 책임지는가?

### 같은 시스템을 세 관점으로 보기

> **서로 다른 View의 요소는 1:1로 대응할 필요가 없다.**

하나의 모듈로 여러 서비스 인스턴스를 만들 수 있고, 여러 모듈을 하나의 서비스로 패키징할 수도 있다.

![같은 온라인 서점을 세 관점으로 보기](diagrams/09-three-viewtypes-simple.png)

## 4. 필요한 View는 이해관계자의 질문에서 결정된다

> **필요한 View는 이해관계자의 질문을 기준으로 선택해야 한다.**

“모든 View를 만들면 안전하다”는 생각은 문서 비용과 중복을 늘린다. 반대로 팀이 익숙한 다이어그램만 만들면 중요한 질문이 빠진다. 따라서 View 선택은 다음 인과관계를 따른다.
...
![이해관계자 중심 View 선택](diagrams/10-select-views-simple.png)

다음 표처럼 질문과 View의 연결을 먼저 작성하면 불필요한 그림을 줄일 수 있다.

| 이해관계자 질문 | 필요한 정보 | 우선 View |
|---|---|---|
| 결제 변경 시 어떤 코드가 바뀌는가? | 정적 의존성 | 모듈 uses View |
| 알림 장애가 주문에 전파되는가? | 런타임 호출과 이벤트 | C&C View |
| DB 장애 시 대기 노드가 있는가? | 노드와 복제 관계 | 배포 View |
| 주문 기능은 어느 팀이 담당하는가? | 책임 대응 | 작업 배정 View |

## 5. 선택한 View를 오해 없이 표현한다

> **View의 상자와 화살표는 의미를 명확히 정의해야 한다.**
> **표기법은 View의 목적에 필요한 차이만 명확히 드러내야 한다.**
...
![모호한 화살표를 피하는 법](diagrams/07-avoid-ambiguous-arrows.png)

모든 정보를 한 그림에 넣을 필요는 없다. 동기 요청과 비동기 이벤트를 구분하는 것이 목적이라면 그 차이만 명확히 표현하고, 상세 오류 코드는 인터페이스 문서로 분리한다.

## 6. 그림을 설명 자료로 완성한다

> **다이어그램은 설명 자료를 더해야 완전한 View가 된다.**

다이어그램은 View의 입구이지 전체 문서가 아니다. 그림만으로는 요소의 책임, 제약, 인터페이스, 설계 이유를 충분히 전달할 수 없다. 따라서 하나의 View는 다음 순서로 완성한다.

![완전한 View를 구성하는 설명 자료](diagrams/18-complete-view.png)
...
C&C View는 어떤 요소가 연결될 수 있는지 보여주지만, 실제 메시지 순서와 조건까지 모두 보여주지는 않는다.
...
![주문 생성 성공 및 결제 실패 동작](diagrams/06-order-sequence.png)

## 9. 여러 View를 연결해 하나의 시스템으로 만든다

> **같은 시스템을 설명하는 여러 View를 하나의 전체로 이해하려면 View 간 요소의 대응 관계를 명시해야 한다.**

PDF는 View 간 매핑을 문서화할 때 한 View의 각 요소에 대응하는 다른 View의 요소를 기록하고, 그 대응이 부분적인지 전체적인지도 밝히라고 권고한다. 대응은 항상 1:1이 아니다. 하나의 모듈로 여러 런타임 인스턴스를 만들 수 있고, 여러 모듈을 하나의 런타임 요소로 패키징할 수도 있다.
...
![모듈, 런타임, 배포 View 간 요소 매핑](diagrams/26-view-mapping.png)
...
4. 질문에 맞춰 모듈, C&C, 할당 View를 선택한다.
5. 선택한 View의 요소와 관계를 범례로 명확히 정의한다.
6. 그림을 요소 목록, 인터페이스, 설계 근거로 보완한다.
7. 정적 구조로 설명되지 않는 순서와 조건은 동작 모델로 기록한다.
8. View 간 매핑으로 코드 변경부터 배포 영향까지 추적한다.
